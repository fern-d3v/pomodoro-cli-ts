...
const pomodoro = {
	...
	currentSession: SessionType.STUDY as SessionType,
	primarySessionType: SessionType.STUDY as SessionType,
...
// Session durations in seconds (60 min dev, 30 min study, 5 min short break, 15 min long break)
...
async function playSound(audioPath: string): Promise<void> {
	try {
		...
		const absolutePath = audioPath.startsWith("/")
			? audioPath
			: `${import.meta.dir}/${audioPath.replace(
					/^\.\//,
					""
	...
	if (pomodoro.currentSession === SessionType.DEV) {
		...
		console.log(
			chalk
				.hex(COLORS.green)
				.bold(
					"Dev session complete! Time for a 15 minute break!"
				)
		);
		startSession(
			SessionType.LONG_BREAK,
			SESSION_DURATIONS.LONG_BREAK
		);
	} else if (
		pomodoro.currentSession === SessionType.STUDY
	) {
		pomodoro.completedWorkSessions++;
		console.log(
			chalk
				.hex(COLORS.green)
				.bold("Study session complete! Great job!")
		);
		...
		if (
			...
			console.log(
				chalk
					.hex(COLORS.teal)
					.bold("Time for a long break! You've earned it!")
			);
			startSession(
				SessionType.LONG_BREAK,
				SESSION_DURATIONS.LONG_BREAK
			...
			console.log(
				chalk
					.hex(COLORS.green)
					.bold("Time for a short break!")
			);
			startSession(
				SessionType.SHORT_BREAK,
				SESSION_DURATIONS.SHORT_BREAK
			);
		}
	} else {
		// Finished a break - return to the primary session type
		const sessionName =
			pomodoro.primarySessionType === SessionType.STUDY
				? "study"
				: pomodoro.primarySessionType === SessionType.DEV
				? "dev"
				: "work";
		console.log(
			chalk
				.hex(COLORS.red)
				.bold(`Break's over! Back to ${sessionName}!`)
		);

		if (pomodoro.primarySessionType === SessionType.STUDY) {
			startSession(
				SessionType.STUDY,
				SESSION_DURATIONS.STUDY
			);
		} else if (
			pomodoro.primarySessionType === SessionType.DEV
		) {
...
function startSession(
	...
	// If starting a STUDY, or DEV session, update the primary session type
	if (
		sessionType === SessionType.STUDY ||
		sessionType === SessionType.DEV
	) {
	...
	const sessionDisplay = getSessionDisplay();
	const duration = chalk.hex(COLORS.yellow)(
...
function startPomodoro(): void {
	...
	console.log(
		chalk.gray(
			"Study: 30  min | Short break: 5 min | Long break: 15 min (every 4 study sessions)"
		)
	);
...
function resumePomodoro(): void {
	if (pomodoro.state === TimerState.PAUSED) {
		pomodoro.state = TimerState.RUNNING;
		console.log(
			chalk.hex(COLORS.green)("[RESUMED] Timer resumed!\n")
		);
	}
}
...
async function showMenu(): Promise<{
	...
	console.log(
		chalk
			.hex(COLORS.mauve)
			.bold("               POMODORO TIMER CLI")
	);
	console.log(chalk.hex(COLORS.mauve).bold("=".repeat(50)));
	console.log(
		chalk.hex(COLORS.subtext1)(
			"Built with TypeScript and Bun ❥"
		)
	);
	console.log();
	...
	console.log(
		chalk.hex(COLORS.text)("Select session type:")
	);
	console.log(
		chalk.hex(COLORS.peach)("  1.") +
			chalk.hex(COLORS.text)(
				" Development Session (30 minutes)"
			)
	);
	console.log(
		chalk.hex(COLORS.peach)("  2.") +
			chalk.hex(COLORS.text)(" Work Session (25 minutes)")
	);
	console.log(
		chalk.hex(COLORS.peach)("  3.") +
			chalk.hex(COLORS.text)(" Study Session (30 minutes)")
	);
	console.log(
		chalk.hex(COLORS.peach)("  4.") +
			chalk.hex(COLORS.text)(" Short Break (5 minutes)")
	);
	console.log(
		chalk.hex(COLORS.peach)("  5.") +
			chalk.hex(COLORS.text)(" Long Break (15 minutes)")
	);
	console.log(
		chalk.hex(COLORS.peach)("  6.") +
			chalk.hex(COLORS.text)(" Custom Duration")
	);
	console.log(
		chalk.hex(COLORS.peach)("  7.") +
			chalk.hex(COLORS.text)(" Exit")
	);
	console.log();

	// Read user input
	process.stdout.write(
		chalk.hex(COLORS.yellow)("Enter choice (1-7): ")
	);
	...
	switch (choice) {
		case "1":
			return {
				type: SessionType.DEV,
				duration: SESSION_DURATIONS.DEV,
			};
		case "2":
			return {
				type: SessionType.STUDY,
				duration: SESSION_DURATIONS.STUDY,
			};
		case "3":
			return {
				type: SessionType.SHORT_BREAK,
				duration: SESSION_DURATIONS.SHORT_BREAK,
			};
		case "4":
			return {
				type: SessionType.LONG_BREAK,
				duration: SESSION_DURATIONS.LONG_BREAK,
			};
		case "5":
			// Custom duration
			process.stdout.write(
			...
			const minutes = await new Promise<string>(
				(resolve) => {
					process.stdin.once("data", (data) => {
						resolve(data.toString().trim());
					});
			...
			if (isNaN(duration) || duration <= 0) {
				console.log(
					chalk.hex(COLORS.red)(
						"Invalid duration. Returning to menu..."
					)
				);
...
async function main(): Promise<void> {
	...
	while (true) {
		...
		console.clear();
		console.log(
			chalk.hex(COLORS.mauve).bold("=".repeat(50))
		);
		console.log(
			chalk.hex(COLORS.mauve).bold("       POMODORO TIMER")
		);
		console.log(
			chalk.hex(COLORS.mauve).bold("=".repeat(50))
		);
		console.log(
			chalk.hex(COLORS.subtext1)(
				"Controls: [SPACE] pause/resume | [Ctrl+C] cancel"
			)
		);
		console.log();
